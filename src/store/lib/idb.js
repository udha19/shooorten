import { setLoader } from "../actions/history.action";
				...
				stores.forEach((store) => {
					if (store.data !== undefined) {
						...
						store.data.forEach((data) => {
							...
							allData.onsuccess = (e) => {
								let match = e.target.result.find((matchedData) => matchedData.id === data.id);

								if (!match) {
									let tx = db.transaction([store.name], 'readwrite');
									let addReq = tx.objectStore([store.name]).add(data);
			...
			if (target) {
				...
				cursorRequest.onsuccess = (event) => {
					...
					if (cursor && new RegExp(key).test(cursor[keyPath])) {
						...
						updateRequest.onsuccess = (event) => getAllObjectData(store).then((storeData) => resolve(storeData));
				...
				reject(new Error(`There's no object in the ${store.name} whose ${keyPath} matches ${key}.`));
			}
		});
	});
};
